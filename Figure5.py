# -*- coding: utf-8 -*-
"""
Created on Mon Aug 17 11:28:53 2026
...
for name, val in zip(features_phys, mean_shap_phys_norm):
    shap_df_list.append({'Feature': translate_feature_name(name), 'SHAP_Value': val, 'Group': 'DesignNet'})
...
def plot_nature_shap_giant_fonts(df, window_label, cell_label):
    ...
    phys_vals = [df[(df['Feature'] == f) & (df['Group'] == 'DesignNet')]['SHAP_Value'].values[0] if len(df[(df['Feature'] == f) & (df['Group'] == 'DesignNet')]['SHAP_Value'].values) > 0 else 0.0 for f in unique_features]
    ...
    ax.bar(x_pos + width/2, phys_vals, width, color=color_virt, edgecolor='#333333', linewidth=0.5, alpha=0.95, hatch='//', label='DesignNet')
...
#%% V2G Voltage Range Distribution (Figures 7d)

file_path = os.path.join('sourcedata', 'V2G_Recovered_Fixed_20260808_1422.pkl')
...
configs = [
    ('PINN_Base', 'PINN'),
    ('PINN_Phys', 'DesignNet'),
    ('CNN_Phys', 'CNN with design parameters')
]
