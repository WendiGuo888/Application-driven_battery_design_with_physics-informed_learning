...
#%% SHAP Analysis for Unseen Fast-Charging Cases (Figure 4a-1)
...
#%% SHAP Analysis for Multi-Step Fast-Charging Cases to Assess the Impact of Design Parameters (Figure 4a-2)
...
#%% Loss Landscape for Assessing the Impact of Design Parameters (Figure 4b)
...
#%% Heterogeneity Plot of Virtual Sensing Features (Figure 4c)
...
#%% EOL Prediction Results and Errors Across 10 Random Seeds (Figure 4d)
...
def plot_eol_comprehensive_analysis_aggregated(seed_eol_absolute):
    ...
    for m_type in m_types:
        ...
        label = "Base" if m_type == "PINN_Base" else "Design states"
...
def plot_academic_summary_bars_final_dual_time(df_results):
    ...
    mode_name_map = {
        "PINN_Base": "Base", "PINN_Phys": "DesignNet",
        "Base": "Base", "Phys": "DesignNet"
    }
    ...
    custom_palette = {"Base": "#5A6096", "DesignNet": "#D6A2BD"}

    legend_handles = [
        Patch(facecolor=custom_palette["Base"], edgecolor="black", alpha=0.30, label="Base"),
        Patch(facecolor=custom_palette["DesignNet"], edgecolor="black", alpha=0.30, label="DesignNet"),
        Line2D([0], [0], color='#2F3561', marker='o', markersize=6, linestyle='--', linewidth=1.5, label='Time [Base]'),
        Line2D([0], [0], color='#A66384', marker='s', markersize=6, linestyle='-', linewidth=2, label='Time [DesignNet]')
    ]
    ...
    for row in range(2):
        for col in range(2):
            ...
            sns.barplot(
                ...
                hue_order=["Base", "DesignNet"], ax=ax, palette=custom_palette,
            ...
            ax2.plot(x_indices + offset, time_stats["DesignNet"], 
...
def plot_percentage_parity_morandi(seed_results, m_base, m_phys):
    ...
    for i, (label, m_name) in enumerate([('PINN', m_base), ('DesignNet', m_phys)]):
        ax = axes[i]
